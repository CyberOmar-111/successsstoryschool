...
class SchoolPortalHandler(BaseHTTPRequestHandler):
    ...
    def handle_register(self, body):
        ...
        with db_connection() as connection:
            ...
            sequence = connection.execute(
                "SELECT value FROM sequences WHERE name = 'student_id'"
            ).fetchone()["value"]
            ...
            connection.execute(
                "UPDATE sequences SET value = ? WHERE name = 'student_id'",
                (next_number,),
            )
            connection.execute(
                """
    ...
    def handle_login(self, body):
        ...
        password = str(body.get("password", ""))
        attempt_key = f"{self.client_address[0]}|{student_id}"
        ...
        cookie = f"sss_session={token}; HttpOnly; SameSite=Strict; Path=/; Max-Age={SESSION_TTL_SECONDS}"
    ...
    def handle_logout(self):
        ...
        self.send_json(
            200,
            {"ok": True},
            {"Set-Cookie": "sss_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"},
        )
    ...
    def handle_admin_login(self, body):
        ...
        password = str(body.get("password", ""))
        attempt_key = f"admin|{self.client_address[0]}|{admin_id}"
        retry_after = self.login_rate_status(attempt_key)
        if retry_after:
            self.send_json(
                429,
        ...
        cookie = f"sss_admin_session={token}; HttpOnly; SameSite=Strict; Path=/; Max-Age={SESSION_TTL_SECONDS}"
        self.send_json(
            200,
    ...
    def handle_admin_logout(self):
        ...
        self.send_json(
            200,
            {"ok": True},
            {"Set-Cookie": "sss_admin_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"},
        )
    ...
    def handle_admin_account_create(self, body):
        ...
        with db_connection() as connection:
            connection.execute("BEGIN IMMEDIATE")
            sequence = connection.execute(
                "SELECT value FROM sequences WHERE name = 'admin_id'"
            ).fetchone()["value"]
            next_number = max(2, sequence + 1)
            admin_id = f"ADM-{next_number}"
            connection.execute(
                "UPDATE sequences SET value = ? WHERE name = 'admin_id'",
                (next_number,),
            )
            connection.execute(
                """
    ...
    def handle_teacher_account_create(self, body):
        ...
        with db_connection() as connection:
            connection.execute("BEGIN IMMEDIATE")
            sequence = connection.execute(
                "SELECT value FROM sequences WHERE name = 'teacher_id'"
            ).fetchone()["value"]
            next_number = sequence + 1
            if next_number > 999:
                connection.rollback()
            ...
            connection.execute(
                "UPDATE sequences SET value = ? WHERE name = 'teacher_id'",
                (next_number,),
            )
            connection.execute(
                """
    ...
    def handle_teacher_login(self, body):
        ...
        password = str(body.get("password", ""))
        attempt_key = f"teacher|{self.client_address[0]}|{teacher_id}"
        retry_after = self.login_rate_status(attempt_key)
        if retry_after:
            self.send_json(
                429,
        ...
        cookie = f"sss_teacher_session={token}; HttpOnly; SameSite=Strict; Path=/; Max-Age={SESSION_TTL_SECONDS}"
    ...
    def handle_teacher_logout(self):
        ...
        self.send_json(
            200,
            {"ok": True},
            {"Set-Cookie": "sss_teacher_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"},
        )
