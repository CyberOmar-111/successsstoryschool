...
test("student registration is pending until school approval", () => {
  ...
  assert.match(server, /email TEXT/);
  ...
  assert.match(js, /emailAddress: "Email address"/);
  assert.match(js, /email_rule: "emailError"/);
  assert.match(js, /email: values\.get\("email"\)/);
...
test("Email MFA is wired as a post-password session gate", () => {
  ...
  assert.match(server, /SMTP_FROM = os\.environ\.get\("SMTP_FROM", SMTP_USERNAME\)/);
  ...
  assert.match(server, /code_hash TEXT/);
  assert.match(server, /DELETE FROM mfa_challenges WHERE expires_at <= \?/);
  assert.match(server, /def send_email_mfa_code\(email, code\):/);
  assert.match(server, /client\.send_message\(message\)/);
  assert.match(server, /def account_email\(self, account_id\):/);
  assert.match(server, /SELECT email FROM students WHERE student_id = \?/);
  assert.match(server, /mfa_code_hash\(challenge_id, code\)/);
  ...
  assert.doesNotMatch(server, /elif request_path == "\/api\/teacher\/mfa":/);
  assert.doesNotMatch(server, /self\.handle_mfa_verify\(body, "teacher"\)/);
  ...
  assert.match(server, /"emailHint": mask_email\(email\)/);
  ...
  assert.doesNotMatch(server, /self\.begin_mfa_challenge\("teacher", teacher_id, attempt_key\)/);
  ...
  assert.match(js, /function showMfaForm\(result, endpoint\)/);
  assert.match(js, /pendingMfaChallenge = \{/);
  assert.match(js, /showMfaForm\(result, "\/api\/auth\/mfa"\)/);
  assert.match(js, /mfaForm\.addEventListener\("submit"/);
  assert.match(js, /api\(pendingMfaChallenge\.endpoint/);
  assert.doesNotMatch(js, /window\.prompt/);
  assert.match(teacherJs, /api\("\/api\/teacher\/login"/);
  assert.doesNotMatch(teacherJs, /api\("\/api\/teacher\/mfa"/);
  assert.doesNotMatch(teacherJs, /window\.prompt/);
...
test("build pipeline uses local React, Vite, Vercel, and Render CI/CD", () => {
  ...
  assert.match(renderConfig, /- key: SMTP_FROM\n        sync: false/);
