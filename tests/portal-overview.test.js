...
test("Twilio Verify MFA is wired as a post-password session gate", () => {
  ...
  assert.match(server, /def lookup_twilio_phone_number\(phone_number\):/);
  assert.match(server, /lookups\.v2\.phone_numbers\(phone_number\)\.fetch\(\)/);
  assert.match(server, /def twilio_lookup_valid_jordanian\(phone_number\):/);
  assert.match(server, /getattr\(lookup, "valid", False\)/);
  assert.match(server, /getattr\(lookup, "country_code", ""\) == "JO"/);
